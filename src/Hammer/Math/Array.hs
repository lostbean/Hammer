...
testShape mtr1 mtr2
    ...
    | otherwise =
        error $
            "[AlgebraLargeMatrix] Sorry I can't do this operation in matrices with different shape: "
                ++ "m1 = "
                ++ show (ixSize $ shape mtr1)
                ++ " and m2 = "
                ++ show (ixSize $ shape mtr2)
         ...
         in
            if n1 == m2
                then Array bd arr
                    ...
                    error $
                        "[AlgebraLargeMatrix] Sorry I can't do matrix multiplication with these shapes: "
                            ++ "m1 = "
                            ++ show (ixSize $ shape mtr1)
                            ++ " and m2 = "
                            ++ show (ixSize $ shape mtr2)
