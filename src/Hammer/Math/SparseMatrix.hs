...
mergeSparse3_X sa sb =
    let
     ...
     in
        Sparse3 sv xym zm md (matrixOrig sa)
...
interM ::
    (VGM.MVector va a, VG.Vector vb a, PrimMonad m) =>
    va (PrimState m) a ->
    Int ->
    [(vb a, Int, Int)] ->
    [(vb a, Int, Int)] ->
...
{-# INLINE moveSlice #-}
moveSlice ::
    (VG.Vector va a, VGM.MVector vb a, PrimMonad m) =>
    va a ->
    vb (PrimState m) a ->
    Int ->
    Int ->
