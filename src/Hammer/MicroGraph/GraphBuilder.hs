...
getMicroVoxel (vboxGID, grainSet) =
    let
...
buildMicroVoxel eset fset mv (p, pid) =
    let
    ...
    let
        go acc pos =
            let
             ...
             in
                case checkVertex validNeigh validEdges of
...
getExtVBR vbr =
    let
     ...
     in
        vbr{vbrDim = VoxBoxDim (dx * 2 - 1) (dy * 2 - 1) (dz * 2 - 1)}
...
getConnFacePos dir p@(FacePos vp) =
    let
        add2p = FacePos . (vp #+#)
