...
getAlterVertexID (VertexID a b c d) =
    let
...
instance Hashable FaceID where
    ...
    hashWithSalt i (AlterFaceID is a b) =
        let
            salt = i `hashWithSalt` is
         in
            hashWithSalt salt (a, b)

instance Hashable EdgeID where
    hashWithSalt i (EdgeID a b c) = hashWithSalt i (a, b, c)
    hashWithSalt i (MultiEdgeID s) =
        let
            salt = i `hashWithSalt` (332 :: Int)
         in
            S.foldl' hashWithSalt salt s
    hashWithSalt i (AlterEdgeID is a b c) =
        let
            salt = i `hashWithSalt` is
         in
            hashWithSalt salt (a, b, c)
    hashWithSalt i (AlterMultiEdgeID is s) =
        let
            salt = i `hashWithSalt` is
         in
            S.foldl' hashWithSalt salt s
...
instance Hashable VertexID where
    ...
    hashWithSalt i (MultiVertexID s) =
        let
            salt = i `hashWithSalt` (665 :: Int)
         in
            IS.foldl' hashWithSalt salt s
    hashWithSalt i (AlterVertexID is s) =
        let
            salt = i `hashWithSalt` is
         in
            IS.foldl' hashWithSalt salt s
