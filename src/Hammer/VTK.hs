...
renderFacePos (VoxBoxExt vbox) face =
    let
...
renderEdgePos (VoxBoxExt vbox) edge =
    let
        e p = case getVoxelID (dimension vbox) p of
            Just x -> x
...
getExtendedVoxBox vbox =
    let
        func (VoxBoxDim x y z) = VoxBoxDim (x + 1) (y + 1) (z + 1)
        range =
            let vbr = dimension vbox
             in vbr{vbrDim = func $ vbrDim vbr}
     in
        VoxBoxExt $ vbox{dimension = range}
...
getVoxBoxCornersPoints (VoxBoxExt vbox) =
    let
        ...
        foo i =
            let
                ...
                x = ry
             in
                evalVoxelPos vbox (origin #+# VoxelPos x y z)
     in
        U.generate size foo
...
renderVoxBoxVTK vbox attrs =
    let
     ...
     in
        mkRLGVTK "VoxBoxPoints" vx vy vz attrs

renderVoxElemListVTK ::
    (RenderCell (VTKElem elem), RenderVox elem) =>
    VoxBox a -> [elem] -> VTK Vec3D
renderVoxElemListVTK vbox v =
    let
     ...
     in
        mkUGVTK "MicroGraph" ps rs [] [attr]

renderVoxElemVTK ::
    (Unbox elem, RenderCell (VTKElem elem), RenderVox elem) =>
    VoxBox a -> [V.Vector elem] -> VTK Vec3D
renderVoxElemVTK vbox v =
    let
        vbext = getExtendedVoxBox vbox
        ps = getVoxBoxCornersPoints vbext
...
renderAllElemProp ::
    (RenderCell (VTKElem elem), RenderVox elem, HasPropValue prop) =>
    VoxBox a -> [prop (V.Vector elem)] -> VTK Vec3D
renderAllElemProp vbox vec =
    let
        ps = mapMaybe getPropValue vec
     in
        renderVoxElemVTK vbox ps
...
renderMicroVertexVTK vbox mv =
    let
        ps = mapMaybe getPropValue (HM.elems $ microVertex mv)
     in
        renderVoxElemListVTK vbox ps
