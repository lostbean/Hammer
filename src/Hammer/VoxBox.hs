...
import Hammer.VoxBox.Types hiding (Vector)
import Linear.Vect
...
unsafeGetVoxelID VoxBoxRange{..} pos =
    let
        (VoxBoxDim maxX maxY _) = vbrDim
        (VoxelPos x y z) = pos #-# vbrOrigin
     in
        abs x + abs (maxX * y) + abs (maxX * maxY * z)
...
evalDisplacedVoxelPos vbox dir vpos =
    let
...
evalVoxelPos vbox (VoxelPos x y z) =
    let
        VoxBoxOrigin ix iy iz = origin vbox
        VoxelDim dx dy dz = spacing vbox
...
evalCentralVoxelPos vbox (VoxelPos x y z) =
    let
        VoxBoxOrigin ix iy iz = origin vbox
        VoxelDim dx dy dz = spacing vbox
...
findIntersection ::
    (Eq a, DotProd a u, UnitVector a v u) =>
...
getBoxLinRange (VoxBoxRange (VoxelPos lx ly lz) (VoxBoxDim dx dy dz)) =
    let
        ux = func dx lx
        uy = func dy ly
        uz = func dz lz
...
splitInTwoBox box@(VoxBoxRange org boxdim)
    ...
    | otherwise =
        let
            ...
            divDim dir (VoxBoxDim dx dy dz)
                | dir == XDir =
                    let
                        x = dx `quot` 2
                     in
                        (VoxBoxDim x dy dz, VoxBoxDim (dx - x) dy dz)
                | dir == YDir =
                    let
                        y = dy `quot` 2
                     in
                        (VoxBoxDim dx y dz, VoxBoxDim dx (dy - y) dz)
                | otherwise =
                    let
                        z = dz `quot` 2
                     in
                        (VoxBoxDim dx dy z, VoxBoxDim dx dy (dz - z))
         ...
         in
            return (VoxBoxRange org dim1, VoxBoxRange org2 dim2)
...
posSet2VoxBox ps =
    let
     ...
     in
        posRange2VoxBox v1 v2

{-# INLINE posRange2VoxBox #-}
posRange2VoxBox :: VoxelPos -> VoxelPos -> VoxBoxRange
posRange2VoxBox (VoxelPos x1 y1 z1) (VoxelPos x2 y2 z2) =
    let
...
isInBox box (VoxelPos x y z) =
    let
        (lx, ux, ly, uy, lz, uz) = getBoxLinRange box
     in
        x >= lx && x <= ux && y >= ly && y <= uy && z >= lz && z <= uz

isOnEdge :: VoxBoxRange -> VoxelPos -> Bool
isOnEdge box (VoxelPos x y z) =
    let
        (lx, ux, ly, uy, lz, uz) = getBoxLinRange box
     in
        x == lx || x == ux || y == ly || y == uy || z == lz || z == uz
...
sizeVoxBoxRange =
    let
     ...
     in
        func . getVoxBoxDim . vbrDim

getRangePos :: VoxBoxRange -> [VoxelPos]
getRangePos VoxBoxRange{..} =
    let
...
toEdgeVoxelPos f@(EdgePos (VoxelPos x y z))
  ...
  where
    p = VoxelPos (div x 2) (div y 2) (div z 2)
