...
resetGrainIDs (vb, hmap) =
    let
     ...
     in
        (vb{grainID = U.map foo (grainID vb)}, map2)
...
grainFinder isEqual vb@VoxBox{..} =
    let
        found = finderVoxConn isEqual grainID dimension
     in
        case (voxConnMap found, voxConnList found) of
            (Just m, Just l) ->
                let
                    new_vec = U.map mkGrainID m
                 in
                    return (vb{grainID = new_vec}, l)
            _ -> Nothing
...
mixMaps db@VoxConn{..} conn = case (voxConnMap, voxConnList) of
    (Just m, Just l) ->
        let
         ...
         in
            db{voxConnMap = return new_m, voxConnList = return new_l}
    _ -> db
...
-}
commitConn ::
    (HasVoxConn c g, GridConn g) =>
    VoxBoxRange ->
    (c Int, HashMap Int (Vector g)) ->
    (g, g) ->
    (c Int, HashMap Int (Vector g))
commitConn range maps@(connMap, connList) (p1, p2) =
    let
...
scanWall dir (WallBoxRange br) =
    let
        (lx, ux, ly, uy, lz, uz) = getBoxLinRange br
     in
        V.fromList $ case dir of
