                ...
                getEither i =
                    let
                     ...
                     in
                        getRightBox (range %# i)
             in
                return $ G.generate size getEither
     in
        do
...
updateDB_VecG ::
    ...
    , G.Vector v a
    , GridConn g
    , G.Mutable v ~ G.Mutable v2
    ) =>
    v1 g -> a -> VoxBoxRange -> v a -> v2 a
updateDB_VecG set gid vbr vec =
    let
     in runST $ do
...
updateDB_HM ::
    ...
    Int ->
    VoxBoxRange ->
    HashMap VoxelPos Int ->
    HashMap VoxelPos Int
updateDB_HM set gid _ hm =
    let
...
mergeHMBox d1 d2 =
    let
     ...
     in
        mergeMaybe func (voxConnMap d1) (voxConnMap d2)
...
updateDB_HM_Int ::
    ...
    HashMap Int Int ->
    HashMap Int Int
updateDB_HM_Int set gid range hm =
    let
        func acc p =
            let
                pos = range %@ (toVoxelPos p)
             in
                HM.adjust (const gid) pos acc
     in
        V.foldl' func hm set
...
updateDB_IM_Int ::
    ...
    IntMap Int ->
    IntMap Int
updateDB_IM_Int set gid range hm =
    let
        func acc p =
            let
                pos = range %@ (toVoxelPos p)
             in
                IM.adjust (const gid) pos acc
     in
        V.foldl' func hm set
